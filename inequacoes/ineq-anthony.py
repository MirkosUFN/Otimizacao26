...
def obter_inequacoes():
    """
    inequações na forma a1X + a2Y SINAL valorDireito.
    Returna dicionários com as inequações.
    """
    inequacoes = []
    operadores_validos = ['<', '<=', '>', '>=']
    ...
    try:
        ...
        for i in range(n):
            print(f"\nInequação {i+1}:")
            a1 = float(input("coeficiente  X (a1): "))
            a2 = float(input("coeficiente  Y (a2): "))
...
def plotar_sistema_inequacoes(inequacoes):
    """
    Plota as inequações no primeiro quadrante, destacando a intersecção das regiões.
    
    Parâmetros:
    inequacoes (list): dicionários de inequações.
    """
    if not inequacoes:
